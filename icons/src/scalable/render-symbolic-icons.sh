...
##     -c, --context <name>     Optimize ALL the icons in icons/src/<context> and save them under icons/Yaru-remix/<context>
##     -f, --file <name>        Optimize only the icon in icons/src/<context>/<path> and save it under icons/Yaru-remix/<context>/<name> (needs --context)
##
## NOTE:
...
# current workding directory expected to be Yaru-remix/icons/src/scalable
CWD=$(pwd)
if [[ ! ${CWD} =~ "icons/src/scalable" ]]; then
  fatal "unexpected working directory ${CWD}. Please execute the script under Yaru-remix/icons/src/scalable folder."
fi
...
optimize() {
  ...
  OUTDIR=../../Yaru-remix/scalable/${GROUP}/
  OUTPUT=../../Yaru-remix/scalable/${GROUP}/${NAME}
